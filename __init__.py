...
class Dataset:
    ...
    def shuffle(self):
        ...
        random.shuffle(data)
        self.images, self.labels = [list(v) for v in zip(*data)]

    def __len__(self):
        """Returns the number of images in the currently loaded dataset."""
    ...
    def points_near_label(self, min_dist=None, max_dist=None, number=1):
        ...
        """
        cache = getattr(self, '_points_near_cache', False)
        if cache:
            clabel, cmin_dist, cmax_dist, nonz = cache
            if clabel == self.current_label and cmin_dist == min_dist and cmax_dist == max_dist:
        ...
        self._points_near_cache = (self.current_label, min_dist, max_dist, nonz)
        samples = nonz[random.sample(range(nonz.shape[0]), number)]
        return [tuple(p) for p in samples]
    ...
    def label_at(self, coords):
        ...
        for x, y in coords:
            ...
            if x not in x_bounds or y not in y_bounds:
                ...
                print("Warning: sample point outside of image bounds!", 'x', x, 'y', y, 'label_image.shape', label_image.shape)
            else:
    ...
    def compare_label_samples(self, s1, s2):
        ...
        s2[unlbl] = 0
        # How to compare PDFs?
        return np.nan_to_num(np.sum(np.minimum(s1, s2)))

    def collect_reward(self, sample_points, labels=None, classification=None):
        """
        Returns a value in the range [0, 1]
        """
        denom = 10
        if self.cursor != self._reward_cursor:
            self._reward_cursor = self.cursor
            self.label_at([])
            label_path, label_image = self._label_at_cache
            reduced_size = [int(dim / denom) for dim in label_image.shape]
            self._unclaimed_rewards = np.ones(reduced_size, dtype=np.float32)
            # No rewards in unlabeled areas.
            self._unclaimed_rewards[label_image == 0] = 0

        assert(labels is not None and classification is not None)   # TODO: make these optional arguments in case the caller doesn't have them computed...
        accuracy      = self.compare_label_samples(labels, classification)
        sample_points = [(int(x / denom), int(y / denom)) for x, y in sample_points]
        sample_index  = tuple(zip(*sample_points))
        reward_sample = self._unclaimed_rewards[sample_index]
        reward        = accuracy * reward_sample / len(sample_points)
        reward_sample = max(0, reward_sample - reward)
        self._unclaimed_rewards[sample_index] = reward_sample
        return reward

    def _compare_label_samples_background(self, s1, s2):
        """???"""
        assert(s1.shape == s2.shape)
        s1_sum = np.sum(s1)
        s2_sum = np.sum(s2)
        ...
        for label_path in self.labels:
            ...
            for label_color in labels_used:
                try:
                    ...
                    err_msg = 'Error: label image %s has color %s which is not in %s/labels.csv.'%(
                                label_path, str(label_color), self.path)
                    print(err_msg)
                    s = err_msg + '\n\n' + s
                    continue
    ...
    def discard_certain_labels(self, discard):
        """
        Removes all images which contain only the given labels, or are
        unlabeled.
        """
        print("Discarding all images which are unlabeled or contain only the following labels:",
            ', '.join(discard))
        self.images = [im for idx, im in enumerate(self.images) if self.labels[idx] is not None]
        self.labels = [lbl for lbl in self.labels if lbl is not None]

        label_colors   = {name : color for color, name in self.names.items()}
        discard_colors = [label_colors[lbl] for lbl in discard]
        for idx in range(len(self.labels)-1, -1, -1):
            # Load each label file and discard the ones which are all zero or
            # which only contain the discarded labels.
            lbl = np.array(Image.open(self.labels[idx]), copy=True)
